...
RegisterTunnel.getInfos = function()
    ...
    if user_id then
        ...
        for k,v in pairs(Config.Orgs) do
            if vRP.HasGroup(user_id, v.permission) then
                org = k
                open[user_id] = k
        ...
        if announces[type] then
            ...
            for k,v in pairs(table) do
                if v.org == data.org and not v.deleted then
                    TriggerClientEvent('Notify', source, 'vermelho',"Sua organização já possui um anuncio ativo, exclua o atual para poder criar um novamente.", 5000)
                    return false
                end
            end
            local cds = Config.Orgs[data.org].cds
            local request = vRP.Request(source, 'Você deseja usar sua localização atual? ( caso não, será utilizado a localização da sua fac ou org )', 'Sim', 'Nao')
            if request then
                local loc = GetEntityCoords(GetPlayerPed(source))
                cds = vec3(loc.x, loc.y, loc.z)
                TriggerClientEvent( "Notify", source, "verde", "Você enviou o anuncio do recrutamento com sucesso.", 5000 )
            end
            table[#table+1] = {
                id = #table+1,
                org = data.org,
                desc = data.description,
                author = identity.name.." "..identity.name2,
                cds = cds,
                loc = true,
                deleted = false
            }
    ...
    if user_id then
        ...
        if announces[type] then
            if announces[type][data.announce.id] then
                ...
                TriggerClientEvent( "Notify", source, "verde", "Você deletou o anuncio do recrutamento com sucesso.", 5000 )
                 -- print("enviou aqui")
            end
            return true
        end
